...
export const returnsController = async (req, res) => {
  ...
  return res.status(400).json({
    ...
    message: "Missing request body",
        ...
        const { orderId, sellerId, reason, refundAmount, status } = req.body;
        const userId = req.user._id;

        const newReturn = new returnModel({ orderId,sellerId, userId, reason, refundAmount, status  });
...
export const getAllReturnsBySellerId = async (req, res) => {
    try {
        ...
        const returns = await returnModel.find({ sellerId }).populate('orderId');
